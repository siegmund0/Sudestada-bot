...
module.exports = {
    ...
    run: async(client, message, args) => {
        if (args) {
            let resolveConArgs = await Gamedig.query({
                type: 'dayz',
                host: `${args}`
            }).then((stateConArgs) => {
                const inline = true;
                const embedConArgs = new Discord.MessageEmbed()
                    .setColor("#C7CED4")
                    .setAuthor(message.author.username, "https://cdn.discordapp.com/avatars/" + message.author.id + "/" + message.author.avatar + ".png")
                    .addFields({ name: "Nombre del servidor", value: '```' + `${stateConArgs.name}` + '```', inline }, { name: "Usuarios", value: '```' + `Online: ${stateConArgs.raw.numplayers} \nMax players: ${stateConArgs.maxplayers}` + '```', inline }, { name: 'Latencia', value: '```' + `Ping: ${stateConArgs.ping}` + '```', inline }, )
                    .setFooter(`2020 © Sudestada ARG - Develop by oaki`)
                    .setTimestamp()
                message.channel.send(embedConArgs);
            }).catch((error) => {
                console.log(error);
            });
        } else {
                ...
                let resolve = await Gamedig.query({
                    ...
                    const embed = new Discord.MessageEmbed()
                        ...
                        .addFields(
                            ...
                            { name: "Usuarios", value: '```' + `Online: ${state.raw.numplayers} \nMax players: ${state.maxplayers}` + '```', inline }, { name: 'Latencia', value: '```' + `Ping: ${state.ping}` + '```', inline },
